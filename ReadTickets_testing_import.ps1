...
## The function for Get-ExcelSheetInfo and Import-Excel, and EPPlus.dll is taken from https://github.com/dfinke/ImportExcel.

$AssemblyPath = "$PSpath\ImportExcel\EPPlus.dll"
...
######################################################################
   ...
   $importB.Add_Click({ 
        ...
        if ( $importFile.FileName -like "*xlsx" ) {
            ...
            if ( !(Test-Path -Path "$PSpath\ImportExcel\EPPlus.dll") ) {
                Write-Host "
                            Missing module ImportExcel from Doug Finke: https://github.com/dfinke/ImportExcel#
                            Trying to install module.
                            It will take 5 minutes.
                           "
                $ProgressBar = showProgressBar -show $true
                #Install-Module -Name ImportExcel -Scope CurrentUser -Confirm:$false -force

                New-Item -Path "$PSpath\ImportExcel" -ItemType Directory
                Invoke-WebRequest -Uri "https://github.com/dfinke/ImportExcel/raw/refs/heads/master/EPPlus.dll" -OutFile "$PSpath\ImportExcel\EPPlus.dll" 
               
                closeProgressBar -ProgressBar $ProgressBar -Show $true
            }
            ...
            if ( Test-Path -Path "$Global:Path\configureImport.json" ) {
                ...
                if ( $saveBindning.Sheet -or $saveBindning.Title -or $saveBindning.Error -or $saveBindning.Name -or $saveBindning.Prio -or $saveBindning.deadLine -or $saveBindning.createDate ) {
                    ...
                    Write-Host "This file contains an error. The columns are probably empty."
                }
            }
