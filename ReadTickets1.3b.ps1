...
$Global:firstDedalLine = 8
$Global:secondDedalLine = 4
$Global:thirdDedalLine = 0
...
if ( !$(Test-Path -Path "$Global:Settings\Userprofile.json" -ErrorAction SilentlyContinue) ) {
    ...
    $item | Add-Member -type NoteProperty -Name 'firstDeadLine' -Value 8
    $item | Add-Member -type NoteProperty -Name 'secondDeadLine' -Value 4
    $item | Add-Member -type NoteProperty -Name 'thirdDeadLine' -Value 0
...
function loadAutosaveSettings () {
    ...
    #$Global:Path = $AutoSave.path
    $Global:firstDedalLine = $AutoSave.firstDeadLine
    $Global:secondDedalLine = $AutoSave.secondDeadLine
    $Global:thirdDedalLine = $AutoSave.thirdDeadLine
} 
...
function autosaveSettings () {
    ...
    $item | Add-Member -type NoteProperty -Name 'firstDeadLine' -Value $Global:firstDedalLine
    $item | Add-Member -type NoteProperty -Name 'secondDeadLine' -Value $Global:secondDedalLine
    $item | Add-Member -type NoteProperty -Name 'thirdDeadLine' -Value $Global:thirdDedalLine
...
<Window x:Class="TicketSystem.Settings"
        ...
        mc:Ignorable="d"
        Title="Settings" Height="280" Width="383">
    <Grid>
        <Border Background="White" CornerRadius="10" Padding="10" Margin="10">
            <StackPanel>
                ...
                <TextBlock Text="Deadline:" FontSize="14" FontWeight="Normal" />
                <StackPanel Orientation="Horizontal" Margin="0,0,0,0">
                    <TextBlock Text="First deadline:" FontSize="14" FontWeight="Bold" Margin="0,0,0,0" />
                    <Button Width="20.5" Margin="29.5,0,0,0" Background="#FFB1E7F3"  />
                    <TextBox Name="firstDeadlineT" TextWrapping="Wrap" Text="8" Width="30" Margin="10,0,0,0"/>
                    <TextBlock Text="days" FontSize="14" FontWeight="Normal" Margin="4,0,0,0" />
                </StackPanel>
                <StackPanel Orientation="Horizontal" Margin="0,0,0,0">
                    <TextBlock Text="Second deadline:" FontSize="14" FontWeight="Bold" Margin="0,0,0,0" />
                    <Button Width="21" Margin="10,0,0,0" Background="#FFF0F3B1" />
                    <TextBox Name="secondDeadlineT" TextWrapping="Wrap" Text="4" Width="30" Margin="10,0,0,0"/>
                    <TextBlock Text="days" FontSize="14" FontWeight="Normal" Margin="4,0,0,0" />
                </StackPanel>
                <StackPanel Orientation="Horizontal" Margin="0,0,0,0">
                    <TextBlock Text="Third deadline:" FontSize="14" FontWeight="Bold" Margin="0,0,0,0" />
                    <Button Width="21" Margin="23,0,0,0" Background="#FFF3B1B1"  />
                    <TextBox Name="thirdDeadlineT" TextWrapping="Wrap" Text="0" Width="30" Margin="10,0,0,0"/>
                    <TextBlock Text="days" FontSize="14" FontWeight="Normal" Margin="4,0,0,0" />
                </StackPanel>
                <!-- Kontrollknappar -->
        ...
        <ComboBox HorizontalAlignment="Left" Margin="560,162,0,0" VerticalAlignment="Top" Width="120"/>
    </Grid>
</Window>
"@
    ...
    try {
        ...
        $firstDeadlineT = $Window.FindName("firstDeadlineT")
        $secondDeadlineT = $Window.FindName("secondDeadlineT")
        $thirdDeadlineT = $Window.FindName("thirdDeadlineT")
    ...
    $firstDeadlineT.Text = $Global:firstDedalLine
    $secondDeadlineT.Text = $Global:secondDedalLine
    $thirdDeadlineT.Text = $Global:thirdDedalLine
    ...
    $saveB.Add_Click({
        ...
        $Global:firstDedalLine = $firstDeadlineT.Text
        $Global:secondDedalLine = $secondDeadlineT.Text
        $Global:thirdDedalLine = $thirdDeadlineT.Text
        autosaveSettings
        searchForTickets
        $Window.Hide()
    })
    ...
    if ( !$notsolvedR.IsChecked -or $solvedR.IsChecked ) {
        ...
        foreach ($item in $tickets.Items) {
            if ( ![string]::IsNullOrEmpty($item.Deadline) ) {
                ...
                if ( $limit -le $Global:firstDedalLine -and $limit -ge $Global:secondDedalLine ) {
                ...
                } elseif ( $limit -le $Global:secondDedalLine -and $limit -ge $Global:thirdDedalLine ) {
                    $container = $tickets.ItemContainerGenerator.ContainerFromItem($item)
                    if ( $container -ne $null ) { 
                ...
                } elseif ( $limit -le $Global:thirdDedalLine  ) {
                    $container = $tickets.ItemContainerGenerator.ContainerFromItem($item)
                    if ( $container -ne $null ) { 
